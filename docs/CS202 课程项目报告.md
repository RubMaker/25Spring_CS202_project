...
- **软件方案**：通过软件指令序列（如调用库函数、用整数运算模拟浮点操作）间接实现功能，依赖CPU现有指令集。
...
- **硬件方案**：专用电路可并行处理浮点运算的各阶段（如指数对齐、尾数相加、结果规格化），单周期内完成运算，延迟极低（通常1-2个时钟周期）。
...
结合单周期CPU的设计场景（通常面向教育或简单嵌入式应用），**推荐软件方案为主，硬件方案为辅**：

- **选择软件方案**：若目标系统资源有限（如FPGA逻辑单元不足）或需支持灵活的浮点运算调整（如自定义舍入模式），软件方案更合适。
- **补充硬件方案**：若性能是关键需求（如实时信号处理），且资源允许（如FPGA有剩余逻辑单元），可扩展浮点ALU以提升运算速度。
